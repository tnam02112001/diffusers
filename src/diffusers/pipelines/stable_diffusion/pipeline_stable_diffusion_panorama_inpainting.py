...
class StableDiffusionPanoramaInpaintPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin,  FromSingleFileMixin):
    r"""
    Pipeline for panorama inpainting using MultiDiffusion.
            ...
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    if circular_padding:
                        latents_for_view = []
                        masks_for_view = []
                        masked_image_latents_for_view = []
                        image_latents_for_view = [] if  num_channels_unet == 4 else None
                            ...
                            if w_end > latents.shape[3]:
                                ...
                                mask_view = torch.cat(
                                    (
                                        ...
                                        mask[:, :, h_start:h_end, : w_end - mask.shape[3]],
                                    ),
                                    axis=-1,
                                )
                                masked_image_latent_view = torch.cat(
                                    (
                                        masked_image_latents[:, :, h_start:h_end, w_start:],
                                        masked_image_latents[:, :, h_start:h_end, : w_end - masked_image_latents.shape[3]],
                                    ),
                                    axis=-1,
                                )
                                if num_channels_unet == 4:
                                    ...
                                    (
                                        ...
                                        image_latents[:, :, h_start:h_end, : w_end - image_latents.shape[3]],
                                    ),
                                    axis=-1,
                                )
                            else:
                                ...
                                masked_image_latent_view = masked_image_latents[:, :, h_start:h_end, w_start:w_end]
                                if num_channels_unet == 4:
                                    image_latent_view = image_latents[:, :, h_start:h_end, w_start:w_end]
                            latents_for_view.append(latent_view)
                            masks_for_view.append(mask_view)
                            masked_image_latents_for_view.append(masked_image_latent_view)
                            if num_channels_unet == 4:
                                image_latents_for_view.append(image_latent_view)
                        latents_for_view = torch.cat(latents_for_view)
                        masks_for_view = torch.cat(masks_for_view)
                        masked_image_latents_for_view = torch.cat(masked_image_latents_for_view)
                        if num_channels_unet == 4:
                        ...
                        masked_image_latents_for_view = torch.cat(
                            [
                    ...
                    if num_channels_unet == 4:
                        init_latents_proper = image_latents_for_view
                        if do_classifier_free_guidance:
                            init_mask, _ = masks_for_view.chunk(2)
                        else:
                            init_mask = masks_for_view
                        ...
                        latents_denoised_batch = (1 - init_mask) * init_latents_proper + init_mask * latents_denoised_batch
